...
#define _HMITools_VERSION 1.0.0
#endif
...
{	unsigned long lenAscii;
	...
	struct DTStructure errTime;
} ArEventLogREAD;

typedef struct LogView_GetAlarm
{	struct ArEventLogRead ReadItem;
	struct ArEventLogReadAddData ReadAddData;
	struct ArEventLogREAD ReadData;
	struct UtcDT_TO_LocalDTStructure convertUTC;
...
{	struct ArEventLogGetIdent GetIdent;
	struct ArEventLogGetLatestRecordID GetRecordID;
...
_BUR_PUBLIC unsigned long LogViewerGetAlarmString(struct LogView_GetAlarm* t, ArEventLogRecordIDType recordId, ArEventLogIdentType LoggerIdent, plcstring* OutputString);
_BUR_PUBLIC plcbit LogViewerCreateString(struct ArEventLogREAD* ReadData, unsigned char ASCIIData[256], plcstring* OutputString);
